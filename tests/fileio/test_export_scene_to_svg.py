...
def test_scene_to_svg_exporter_get_user_input(view):
    ...
    item2 = ZeePixmapItem(
        QtGui.QImage(100, 100, QtGui.QImage.Format.Format_RGB32))
    item2.setPos(QtCore.QPointF(200, 0))
...
def test_scene_to_svg_exporter_export_writes_svg(view, tmp_path):
    ...
    assert filename.read_bytes().startswith(b'<?xml')
...
def test_scene_to_svg_exporter_export_with_worker(view, tmp_path):
    ...
    worker.progress.emit.assert_called_once_with(0)
    worker.finished.emit.assert_called_once_with(IOResult(filename=filename, errors=[]))
    assert filename.exists()
    assert filename.read_bytes().startswith(b'<?xml')
